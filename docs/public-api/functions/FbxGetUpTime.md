...
> Goal: explicit documentation of the public helper contract of `FbxGetUpTime()`

## Purpose

`FbxGetUpTime()` writes an uptime-style time value into a caller-supplied `struct timeval`.

It is a small public helper function for retrieving uptime or elapsed-runtime style time information through the filesysbox API.
...
`FbxGetUpTime()` is not a lifecycle transition.
...
The function writes the uptime-style time value into this structure.

## Result

`FbxGetUpTime()` does not return a result value.

Its public effect is to store an uptime-style time value in the caller-supplied `struct timeval`.
...
## Relationship to `FbxGetSysTime()`

`FbxGetUpTime()` and `FbxGetSysTime()` are related helper functions, but they report different kinds of time values.
...
`FbxGetUpTime()` operates on an existing filesysbox instance.
...
`FbxGetUpTime()` is a helper query with one direct side effect:
...
A minimal backend does not need `FbxGetUpTime()` in order to mount and run normally.
...
* support for tools or wrappers that want uptime-style time information through the filesysbox API

## Practical rules for callers and backend authors

A caller or backend author should follow these rules:

* call `FbxGetUpTime()` only on a live filesysbox instance
...
`FbxGetUpTime()` must be read together with:

* `FbxGetSysTime()` as the related time helper
...
* the public prototype of `FbxGetUpTime()`
* the caller-supplied `struct timeval` output model
* the distinction between uptime-style time and system time in the public helper surface
